...
import { OrPromise, asArray, computeEntryHash } from "@veramo/utils";
import { bindIfParam, eq } from "drizzle-orm";
import {
...
export class DataStoreDrizzle implements IAgentPlugin {
    ...
    async dataStoreGetMessage(args: IDataStoreGetMessageArgs): Promise<IMessage> {
        ...
        const m = await this.db.query.messages.findFirst({
            // biome-ignore lint: ok
            where: (message: any, { eq }: any) => eq(message.id, args.id),
            // with: {
            //     credentialsToMessages: true,
            //     messagesToPresentations: true,
            // },
        });
        ...
        const m1 = await this.db.query.credentialsToMessages.findMany({
            // biome-ignore lint: ok
            where: (credentialsToMessages: any, { eq }: any) => eq(credentialsToMessages.messageId, args.id),
            with: {
                credential: true,
            },
        });
        ...
        const m2 = await this.db.query.messagesToPresentations.findMany({
            // biome-ignore lint: ok
            where: (messagesToPresentations: any, { eq }: any) => eq(messagesToPresentations.messageId, args.id),
            with: {
                presentation: true,
            },
        });
