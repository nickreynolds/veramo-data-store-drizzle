...
import { credentials, identifiers, keys, presentations, services, } from "../drizzle/schema";
...
export class DIDStoreDrizzle extends AbstractDIDStore {
    ...
    async getDID({ did, alias, provider, }) {
        ...
        if (did && !alias) {
            // biome-ignore lint: ok
            where = (identifier, { eq }) => eq(identifier.did, did); // { did };
        }
        else if (!did && alias) {
            // biome-ignore lint: ok
            where = (identifier, { eq }) => eq(identifier.alias, alias); // { alias };
        }
        else {
            throw Error("[veramo:data-store-drizzle:did-store-drizzle] Get requires did or (alias and provider)");
        }
        ...
        const result = {
            ...
            // biome-ignore lint: ok
            services: id.services.map((service) => {
    ...
    async deleteDID({ did }) {
        const identifier = await this.db.query.identifiers.findFirst({
            // biome-ignore lint: ok
            where: (identifier, { eq }) => eq(identifier.did, did),
            with: {
                keys: true,
                services: true,
    ...
    async importDID(args) {
        ...
        for (const argsService of args.services) {
            const service1 = {
                ...
                serviceEndpoint: typeof argsService.serviceEndpoint === "string"
    ...
    async listDIDs(args) {
        // biome-ignore lint: ok
        let where = (identifier, { eq }) => eq(identifier.provider, args.provider);
        if (args.alias) {
            // biome-ignore lint: ok
            where = (identifier, { eq }) => and(eq(identifier.provider, args.provider), eq(identifier.alias, args.alias));
        }
...
//# sourceMappingURL=did-store-drizzle.js.map